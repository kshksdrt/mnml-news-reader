...
export default function BottomNav({ visibility }) {
  ...
  return (
    <div className={ !visibility ? 'nav-bottom-hide nav-bottom' : ' nav-bottom'}>
      <Link to="/" className={`nav-bottom-item ${location.pathname === '/' && 'nav-tab-active'}`}>
        <Home />
      </Link>
      <Link to="/sources" className={`nav-bottom-item ${location.pathname === '/sources' && 'nav-tab-active'}`}>
        <Sources />
      </Link>
      <Link to="/about" className={`nav-bottom-item ${location.pathname === '/about' && 'nav-tab-active'}`}>
        <About />
      </Link>
    </div>
  )
